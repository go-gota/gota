...
func (e floatElement) Int() (int, error) {
	if e.IsNA() {
		return 0, fmt.Errorf("floatElement.Int(): can't convert NaN to int")
	}
	f := e.e
	if math.IsInf(f, 1) || math.IsInf(f, -1) {
		return 0, fmt.Errorf("floatElement.Int(): can't convert Inf to int")
	}
	if math.IsNaN(f) {
		return 0, fmt.Errorf("floatElement.Int(): can't convert NaN to int")
	}
...
func (e floatElement) Bool() (bool, error) {
	if e.IsNA() {
		return false, fmt.Errorf("floatElement.Bool(): can't convert NaN to bool")
	}
	switch e.e {
	case 1:
		return true, nil
	...
	return false, fmt.Errorf("floatElement.Bool(): can't convert Float \"%v\" to bool", e.e)
}

func (e floatElement) Time() (time.Time, error) {
	return time.Time{}, fmt.Errorf("floatElement.Time(): can't convert float to time.Time")
}
