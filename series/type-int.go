...
func (e intElement) Int() (int, error) {
	if e.IsNA() {
		return 0, fmt.Errorf("intElement.Int(): can't convert NaN to int")
	}
	return int(e.e), nil
}
...
func (e intElement) Bool() (bool, error) {
	if e.IsNA() {
		return false, fmt.Errorf("intElement.Bool(): can't convert NaN to bool")
	}
	switch e.e {
	case 1:
		return true, nil
	...
	return false, fmt.Errorf("intElement.Bool(): can't convert Int \"%v\" to bool", e.e)
}

func (e intElement) Time() (time.Time, error) {
	return time.Time{}, fmt.Errorf("intElement.Time(): can't convert int to time.Time")
}
